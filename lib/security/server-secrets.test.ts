...
describe("getGeminiCredentials", () => {
  ...
  it("falls back to hardcoded stage defaults when no model env vars are set (gemini-2.5-flash was retired for new users - these defaults must never silently regress back to it)", () => {
    vi.stubEnv("GEMINI_API_KEY", "test-key");
    vi.stubEnv("GEMINI_MODEL", "");
    vi.stubEnv("GEMINI_RESEARCH_MODEL", "");
    vi.stubEnv("GEMINI_GENERATION_MODEL", "");

    expect(getGeminiCredentials("research").model).toBe("gemini-2.5-flash-lite");
